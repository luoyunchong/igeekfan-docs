...
1. 未登录，不带 access_token，直接请求需要登录的接口、管理员接口结果一样。
...
2. 携带 access_token，但非超级管理员(admin 字段为 2),访问的方法为角色为超管才有权限的方法。
...
3. 携带 access_token ，访问只需要登录的接口（/cms/user/auths）
...
4. 携带 access_token，但此用户无访问此方法的权限（即该用户的组别未配置此权限）。
...
6. 携带不正常的 access_token 值，后台无法下正常解析出用户信息
